...
Verified by tests (271 Rust tests + 3 hardware-only ignored, 76 frontend
...
1. **Record one real meeting and read the note.** Run `npm run tauri dev`,
   ...
   against.
2. Integrate the in-flight packets: model manager UI (T-M5-3), first-run
   onboarding (T-M5-4), and act on the failure-mode audit.
...
### 2026-08-20 (later) — M2 through M5, five fan-outs
